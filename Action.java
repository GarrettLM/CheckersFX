...
/*	public int getStart() {
		...
		if (attack) {
			strk = new StringTokenizer(this.toString(), "x");
		} else {
			strk = new StringTokenizer(this.toString(), "-");
		}
	...
	public boolean equals(Action move) {
		System.out.print(move.toString() + " " + this.toString() + " = ");
		System.out.println((move.startX == this.startX && move.startY == this.startY && move.destX == this.destX && this.destY == move.destY));
		return (move.startX == this.startX && move.startY == this.startY && move.destX == this.destX && this.destY == move.destY);
	}

	public int[] notationToCoordinates(int n) {
		int[] coord = new int[2];
		switch (n) {
			case 0:
				coord[1] = 0;
